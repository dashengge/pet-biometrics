...
class IterationInit(HookBase):
    def __init__(self, model, data_loader_extract):
        self._logger = logging.getLogger(__name__)
        if isinstance(model, DistributedDataParallel):
            model = model.module
        self.model = model
    ...
    def before_epoch(self):
        print("==============================================init at each epoch=======================================================")
        features, labels = extract_on_dataset(self.model, self.data_loader_extract)
        sameple_feature = torch.stack(features)
        sample_labels = torch.tensor(labels)
        self.model.oim_loss.sample_features = F.normalize(sameple_feature, dim=1).cuda()
        self.model.oim_loss.sample_labels =sample_labels.cuda()
        ...
        # self.model.oim_loss.features = F.normalize(centers, dim=1).cuda()