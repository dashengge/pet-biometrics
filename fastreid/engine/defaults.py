...
from fastreid.evaluation.evaluator import  inference_context,extract_on_dataset
import collections
...
def default_argument_parser():
    ...
    parser.add_argument("--submit-file", default="", help="path to submit csv file")
    parser.add_argument(
        "--resume",
        action="store_true",
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        data_loader = self.build_train_loader(cfg, is_train=True)
        data_loader_extract = self.build_train_loader(cfg, is_train=False)
        self.data_loader_extract = data_loader_extract
    ...
    def build_hooks(self):
        ...
        ret.append(hooks.IterationInit(self.model, self.data_loader_extract))
    ...
    @classmethod
    def build_train_loader(cls, cfg, is_train=True):
        """
        Returns:
            iterable
        ...
        return build_reid_train_loader(cfg, combineall=cfg.DATASETS.COMBINEALL,is_train=is_train)
    ...
    def extract_on_dataset(self, model, data_loader, flip_test=False):
        """
        ...
        logger = logging.getLogger(__name__)
        logger.info("Start extract on {} images".format(len(data_loader.dataset)))
        ...
        with inference_context(model), torch.no_grad():
            for idx, inputs in enumerate(data_loader):
                ...
                for pid, feature in zip(inputs['targets'], outputs):
                    features.append(feature)
                    labels.append(pid.cpu().item())
                # features.append(outputs.data.cpu())
                # labels.append(inputs['targets'])
