...
from fastreid.utils.mask import exchange_token, exchange_patch, get_mask_box, jigsaw_token, cutout_patch, erase_patch, mixup_patch, jigsaw_patch
...
class TransformerHead(EmbeddingHead):
    def __init__(
        self,
        cfg,
    ):
        ...
        trans_names = cfg.MODEL.TRANSFORMER.TRANS_NAMES
        kernel_size = cfg.MODEL.TRANSFORMER.KERNEL_SIZE_2ND
        use_feature_mask = cfg.MODEL.USE_FEATURE_MASK
        ...
        self.conv0 = conv1x1(2048, 2048)
        self.conv1 = conv1x1(2048, d_model)
    ...
    # def from_config(cls, cfg):
        # cfg = cfg
        # trans_names = cfg.MODEL.TRANSFORMER.TRANS_NAMES
        # kernel_size = cfg.MODEL.TRANSFORMER.KERNEL_SIZE
        # mask_shape = cfg.MODEL.TRANSFORMER.MASK_SHAPE
        # mask_size = cfg.MODEL.TRANSFORMER.MASK_SIZE
        # mask_mode = cfg.MODEL.TRANSFORMER.MASK_MODE
        # use_feature_mask = cfg.MODEL.USE_FEATURE_MASK
        # return {
                # "cfg" : cfg,
                # "trans_names": trans_names,
    ...
    def forward(self, box_features, targets=None):
        ...
        # print("bbbbbbb",box_features.shape)
        ...
        pool_feat = self.pool_layer(box_features)
        neck_feat = self.bottleneck(pool_feat)
        neck_feat = neck_feat.view(neck_feat.size(0),-1)

        if not self.training: 
            return pool_feat[..., 0, 0]
