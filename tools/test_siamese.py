...
def main(args):
    ...
    with open("./datasets/pet_biometric_challenge_2022/debug2.csv", "w") as f1:
        with inference_context(model), torch.no_grad():
            ...
            for line in tqdm(data):
                ...
                out_scores = model(input_img)
                # Flip test
                if flip_test:
                    flip_scores = model(input_img_flip)
                    similarity =(out_scores+flip_scores)/2
                else:
                    similarity = out_scores 
                print(similarity)
                f1.write("{},{},{}\n".format(img_path1,img_path2,similarity.detach().item()))
    ...
    # with open("./datasets/pet_biometric_challenge_2022/validation/submit_all_data_rerank.csv", "w") as f1:
    #     for line in tqdm(data):
    #         img_path1, img_path2 = str(line.strip()).split(',')
